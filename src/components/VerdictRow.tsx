...
export function VerdictRow({ row }: { row: VerdictRowData }) {
  return (
    <div className="grid grid-cols-1 gap-2 border-b border-mist py-3 md:grid-cols-[92px_150px_130px_140px_1fr_28px] md:items-center md:gap-4">
      <span className="mono text-[13px] text-fog">{row.time}</span>

      <span className="text-[14px] text-ink">
        {row.member}
        {row.seeded && <span className="ml-2 align-middle"><Pill>seeded</Pill></span>}
      </span>
      ...
      <span className="md:text-right">
        {row.txHash ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mono text-[13px] text-electric hover:underline"
